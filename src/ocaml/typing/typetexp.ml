...
let props_attributes env attrs =
  Ast_helper.map_props
    (fun e ->
       ...
       {e with pexp_desc = Pexp_constant(Ast_helper.Const.string s)}
    )
    attrs
(* END LEXIFI *)
...
let transl_simple_type_with_props env ?univars ~closed styp =
  allow_props := true;
  try_finally
    (fun () -> transl_simple_type env ?univars ~closed styp)
