  ...
  if Current_unit.Name.is_ident id then env
    ...
    let modules =
      (* With [-no-alias-deps], non-material additions should not
         ...
         (See #9991). *)
      if material || not !Clflags.no_alias_deps then
...
let sign_of_cmi ~freshen { Persistent_env.Persistent_signature.cmi; _ } =
  ...
  let mda_declaration =
    Subst.(Lazy.module_decl Make_local id_subst (Lazy.of_module_decl md))
  in
...
let lookup_ident_type ~errors ~use ~loc s env =
  ...
  | exception Not_found ->
      ...
      if not !Clflags.pure_caml && s = "ttype" then
        !lookup_dot_type_ref ~errors ~use ~loc (Location.mknoloc (Lident "Mlfi_types")) (Location.mknoloc "ttype") env
      else
      (* END LEXIFI *)
