...
and compare_variants ~loc env params1 params2 n
  ...
  | cd1::rem1, cd2::rem2 ->
      ...
      else if diff_props cd1.cd_attributes cd2.cd_attributes then [Type_properties]
      else begin
        Builtin_attributes.check_deprecated_inclusion
          ~def:cd1.cd_loc
          ~use:cd2.cd_loc
...
and compare_records ~loc env params1 params2 n
  ...
  | ld1::rem1, ld2::rem2 ->
      ...
      else if ld1.ld_mutable <> ld2.ld_mutable then [Type_properties]
