...
let ttype_type_path = lazy (lookup_type "Mlfi_types.ttype")
...
let type_pattern_list ?bind
    category no_existentials env lazy_flag spatl expected_tys allow
  =
  reset_pattern allow;
  let new_env = ref env in
...
let delayed_dtypes = ref []
let delayed_checks = ref []
let reset_delayed_checks () = delayed_dtypes := []; delayed_checks := []
let add_delayed_dtype ty =
  delayed_dtypes := (ty, Warnings.backup ()) :: !delayed_dtypes
...
let force_delayed_checks () =
  ...
  List.iter
    (fun (ty, w) -> Warnings.restore w;
      try ignore (Dtype.stype_of_type ty) with exn -> Msupport.raise_error exn)
    (List.rev !delayed_dtypes);
  List.iter
    ...
    | Tarrow _, _ ->
        let rec check {exp_desc; exp_loc; exp_extra; _} =
          if List.exists (function
            ...
            | Texp_function _ | Texp_typath _ | Texp_typeof _ ->
                check_statement ()
...
and type_expect_
  ...
  | Pexp_let(rec_flag, spat_sexp_list, sbody) ->
      ...
      let (pat_exp_list, new_env, unpacks) =
        type_let existential_context env ~lazy_flag rec_flag spat_sexp_list true in
  ...
  | Pexp_letmodule(name, smodl, sbody) ->
      ...
      let md =
        { md_type = modl.mod_type; md_attributes = [Ast_helper.Attr.mk (mknoloc "#localmodule#") (PStr[])]; md_loc = name.loc;
  ...
  | Pexp_extension ({txt="t"|"lexifi.t"}, PTyp sty) ->
      ...
      add_delayed_dtype ty;
      rue {
        exp_desc = Texp_typeof ty;
        exp_loc = loc;
        exp_type = newgenty (Tconstr(Lazy.force ttype_type_path, [ty.ctyp_type], ref Mnil));
        exp_extra = [];
        exp_env = env;
        exp_attributes = sexp.pexp_attributes;
      }
      ...
      let e =
        ...
        | Tconstr (p, _, _) ->
            ...
            | Type_record (lbls, _) ->
                ...
                List.fold_right
                  (fun s acc ->
                     let s = mk (Pexp_constant (Pconst_string (s, Location.none, None))) in
  ...
  | Pexp_newtype({txt=name} as label_loc, sbody) ->
      ...
      let decl = new_local_type ~loc ~type_attributes () in
      let scope = create_scope () in
...
and type_let ?(lazy_flag = NonLazy) ?bind
  ...
  let (pat_list, new_env, force, pvs, unpacks) =
    type_pattern_list ?bind Value existential_context env lazy_flag spatl nvs allow in
...
and type_implicit_arg env loc ty =
  ...
  | Auto_ttype t ->
      let e =
        type_expect env (Ast_helper.Exp.extension (Location.mknoloc "lexifi.t", PTyp (Ast_helper.Typ.any ~loc:ghloc ())))
          (mk_expected ty)
      in
