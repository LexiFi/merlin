...
let add_expr_fwd = ref (fun _ _ -> assert false)

let add_props bv l = List.iter (fun (_, e) -> !add_expr_fwd bv e) l
...
let rec add_type bv ty =
  List.iter (add_props bv) (Ast_helper.get_props ty.ptyp_attributes);
...
let add_type_declaration bv td =
  ...
  List.iter (add_props bv) (Ast_helper.get_props td.ptype_attributes);
  let subprops = Ast_helper.type_props td in
  List.iter (add_props bv) subprops;
...
let rec add_expr bv exp =
  ...
  | Pexp_extension ({txt="mlfi.typath"}, p) ->
      addmodule bv (mknoloc (Lident "Mlfi_type_path"));
      let step = function
        | Ast_helper.Typath_constructor (tp, t)
        | Ast_helper.Typath_field (tp, t) ->
            add bv tp; add_opt add_type bv t
        | Ast_helper.Typath_tuple _ -> ()
