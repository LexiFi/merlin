...
let has_lazy_attr =
  List.exists (function ({txt="&lazy&"},_) -> true | _ -> false)

let is_lazy_let e =
  if has_lazy_attr e.pexp_attributes
  then Lazy else NonLazy

let remove_lazy_attr =
  List.filter (function ({txt="&lazy&"},_) -> false | _ -> true)
  ...
  let rec copy ty =
    ...
    with Not_found ->
      ...
      let desc =
        ...
        | Tprop (props, t) -> Tprop (props, copy t)
      in
      ty'.desc <- desc;
      ty'
  in
...
let error (loc, env, err) =
  let err = match err with
    ...
    | Wrong_name (s1, t, s2, p, n, l) ->
      Wrong_name (s1, deep_copy () t, s2, p, n, l)
    ...
    | Too_many_arguments (b, t) ->
      Too_many_arguments (b, deep_copy () t)
    | Abstract_wrong_label (l, t) ->
      Abstract_wrong_label (l, deep_copy () t)
    ...
    | Recursive_local_constraint tr ->
      Recursive_local_constraint (trace_copy tr)
    | err -> err
  in
  Error (loc, env, err)

let level_for_implicit_ttype = ref None

let with_level_for_implicit_ttype f x =
  let old_level_for_implicit_ttype = !level_for_implicit_ttype in
  level_for_implicit_ttype := Some (Ident.current_time());
  try
    let res = f x in
    level_for_implicit_ttype := old_level_for_implicit_ttype;
    res
  with exn ->
    level_for_implicit_ttype := old_level_for_implicit_ttype;
    raise exn
...
let ttype_path =
  Path.(Pdot (Pident ident_mlfi_types, "ttype", nopos))

let call_site_path =
  Path.(Pdot (Pdot (Pident ident_mlfi_types, "StackTrace", nopos), "call_site", nopos))

let typath_type_path =
  Path.(Pdot (Pident ident_mlfi_type_path, "t", nopos))
...
let rec copy_known_part ty =
  ...
  match ty.desc with
  | Tconstr (p, tl, abbrev) ->
      newty2 ty.level (Tconstr (p, List.map copy_known_part tl, abbrev))
  | Tarrow (l, t1, t2, c) ->
      newty2 ty.level (Tarrow (l, copy_known_part t1, copy_known_part t2, c))
  | Ttuple tl ->
      newty2 ty.level (Ttuple (List.map copy_known_part tl))
  | _ ->
      ty
...
let iter_expression f e =
  ...
  let rec expr e =
    ...
    | Pexp_extension ({txt="mlfi.typath"}, p) ->
        List.iter
          (function
            | Ast_helper.Typath_list e | Ast_helper.Typath_array e -> expr e
            | _ -> ()
          )
...
let unify_exp_types loc env ty expected_ty =
  ...
  with
    Unify _trace ->
      begin
        (* Try implicit subtyping *)
        backtrack snap;
        let snap = snapshot () in
        try
          Ctype.subtype env ty expected_ty ();
          if Warnings.is_active (Warnings.Implicit_subtyping ("", "")) then begin
            Printtyp.type_expr Format.str_formatter ty;
            let ty = Format.flush_str_formatter () in

            Printtyp.type_expr Format.str_formatter expected_ty;
            let expected_ty = Format.flush_str_formatter () in

            Location.prerr_warning loc (Warnings.Implicit_subtyping (ty, expected_ty))
          end
        with _ ->
          backtrack snap;
          try unify env ty expected_ty; assert false
          with Unify trace -> raise(error(loc, env, Expr_type_clash(trace)))
             | _ -> assert false
      end
  | Tags(l1,l2) ->
      raise(Typetexp.Error(loc, env, Typetexp.Variant_tags (l1, l2)))
...
and type_pat_aux ~constrs ~labels ~no_existentials ~mode ~explode ~env
  ...
  (* BEGIN LEXIFI *)
  | Ppat_tuple [_] ->
      raise(Typetexp.Error(sp.ppat_loc, !env,
                           Typetexp.Unbound_value(Longident.Lident ",~")))
  (* END LEXIFI *)
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      in
      List.iter
        (function (lab, {ppat_loc = loc; ppat_desc = Ppat_var x; ppat_attributes=attrs})
          when x.txt = list_last (Longident.flatten lab.txt) &&
               not (List.exists (fun ({txt},_) -> txt = "mlfi.punning") attrs) ->
...
let add_pattern_variables ?(lazy_flag = NonLazy) ?(bind = fun _ -> []) ?check ?check_as env =
  let pv = get_ref pattern_variables in
  (List.fold_right
     (fun (id, ty, _name, loc, as_var) env ->
       let check = if as_var then check_as else check in
       let attrs = bind id in
       let attrs =
         match lazy_flag with
         | NonLazy -> remove_lazy_attr attrs
         | Lazy -> lazy_attr :: attrs
       in
...
let type_pattern_list ?bind env lazy_flag spatl scope expected_tys allow =
  ...
  let new_env, unpacks = add_pattern_variables ~lazy_flag ?bind !new_env in
      ...
      | Texp_typath l ->
          List.fold_left (fun use step ->
              ...
              | Ttypath_list e | Ttypath_array e ->
                  Use.join use (Use.guard (expression env e))
            ) Use.empty l
...
(* BEGIN LEXIFI *)
(* Special hack to avoid min/max on "bad" types *)
let check_ident env loc path ty =
  match path with
  | Path.Pdot (Path.Pident m, ("min"|"max"|"<"|">"|"<="|">="|"compare" as op), _) when Ident.global m && Ident.name m = "Pervasives" ->
      add_delayed_check
        (fun () ->
           match (Ctype.repr ty).desc with
           | Tarrow (Nolabel, ty1, _, _) ->
               (* also check for occurrences deep in the type? *)
               begin match (Ctype.expand_head env ty1).desc with
               | Tconstr (p, _, _) ->
                   begin match Path.name p with
                   | "Mlfi_acontract.contract"
                   | "Mlfi_contract.contract" ->
                       raise(Error(loc, env, Min_max_on_bad_type (op, "contract")))
                   | "Mlfi_acontract.obs"
                   | "Mlfi_contract.observable" ->
                       raise(Error(loc, env, Min_max_on_bad_type (op, "observable")))
                   | "Algebra_core.uthing" ->
                       raise(Error(loc, env, Min_max_on_bad_type (op, "uthing")))
                   | "Contract_algebra.thing" ->
                       raise(Error(loc, env, Min_max_on_bad_type (op, "thing")))
                   | "Contract_algebra.contract" ->
                       raise(Error(loc, env, Min_max_on_bad_type (op, "contract")))
                   | "Contract_algebra.value" ->
                       raise(Error(loc, env, Min_max_on_bad_type (op, "value")))
                   | _ -> ()
                   end
               | _ -> ()
               end
...
and type_expect_ ?in_function ?(recarg=Rejected) env sexp ty_expected =
  ...
  | Pexp_ident lid ->
      begin
        ...
        (* BEGIN LEXIFI: lazy-let *)
        if has_lazy_attr desc.val_attributes then
          let mk e = Ast_helper.Exp.mk ~loc:sexp.pexp_loc e in
          let s = Longident.last lid.txt in
          let desc = {desc with val_attributes = remove_lazy_attr desc.val_attributes} in
          let env = Env.store_value (Path.head path) desc env in
          type_expect env (mk (Pexp_apply(mk (Pexp_ident (mknoloc (Longident.parse "Lazy.force"))),
                                          [Nolabel, mk (Pexp_ident (mknoloc (Longident.Lident s)))]))) ty_expected
        else let () = () in
          (* END LEXIFI *)
        let is_recarg =
        ...
        let ty = instance env desc.val_type in
        rue {
          exp_desc =
  ...
  | Pexp_let(Nonrecursive,
    ...
    when contains_gadt env spat && is_lazy_let sexp = NonLazy ->
  ...
  | Pexp_let(rec_flag, spat_sexp_list, sbody) ->
      let lazy_flag = is_lazy_let sexp in
      let scp =
      ...
      let (pat_exp_list, new_env, unpacks) =
        type_let env ~lazy_flag rec_flag spat_sexp_list scp true in
      let body =
  ...
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      List.iter
        (function (lab, {pexp_loc = loc; pexp_desc = Pexp_ident {txt=Longident.Lident x}; pexp_attributes=attrs})
          when x = list_last (Longident.flatten lab.txt) &&
               not (List.exists (fun ({txt},_) -> txt = "mlfi.punning") attrs) ->
            Location.prerr_warning loc Warnings.Missed_punning
                | _ -> ()
        )
      ...
      let md =
        {
          ...
          md_attributes = [mknoloc "#localmodule#", PStr[]];
          md_loc=Location.none
        }
      in
      let id = Ident.create name.txt in
      let new_env = Env.enter_module_declaration id md env in
      (* END LEXIFI *)
      Ctype.init_def(Ident.current_time());
      Typetexp.widen context;
      let body = with_level_for_implicit_ttype (type_expect new_env sbody) ty_expected in
  ...
  | Pexp_extension ({txt="mlfi.typath"}, p) ->
      ...
      let fields = List.map (fun s -> Rtag (Location.mknoloc s, [], true, [])) fields in
      let gamma = {ptyp_desc=Ptyp_variant (fields, Open, None); ptyp_loc=Location.none; ptyp_attributes=[]} in
  ...
  | Pexp_newtype({txt=name}, sbody) ->
      ...
      let decl = {
        ...
        type_loc = loc;
        type_attributes = [mknoloc "#localtype#", PStr[]]; (* LEXIFI *)
      ...
      let body = (*with_level_for_implicit_ttype*) (type_exp new_env) sbody in
...
and type_typath_step env loc alpha beta = function
  | Ast_helper.Typath_constructor (tp, sty) ->
      ...
      | Some sty ->
          let cty = Typetexp.transl_simple_type env false sty in
          unify_exp_types loc env alpha cty.ctyp_type
      end;
      let opath =
        try
          let (p0, p,_) = extract_concrete_typedecl env alpha in
	  Some(p0, p, true)
        with Not_found -> None
      in
      let constrs = Typetexp.find_all_constructors env tp.loc tp.txt in
      let constr = Constructor.disambiguate tp env opath constrs in
      Env.mark_constructor Env.Positive env (Longident.last tp.txt) constr;
      ...
      let t =
        ...
        | l -> newty (Ttuple l)
      in
      unify_exp_types loc env alpha ((*instance env*) ty_res);
      unify_exp_types loc env beta t;
  ...
  | Ast_helper.Typath_field (lid, sty) ->
      ...
      let opath =
        try
          let (p0, p,_) = extract_concrete_record env alpha in
          Some(p0, p, true)
        with Not_found -> None
      in
      ...
      let _, ty_arg, ty_res = instance_label false(*?*) label in
      unify_exp_types loc env alpha ty_res;
      unify_exp_types loc env beta ty_arg;
      Ttypath_field lid
  ...
  | Ast_helper.Typath_tuple (field, arity) ->
      ...
      if arity <= field then
        Syntaxerr.ill_formed_ast loc "field number must be smaller than tuple arity";

      let subtypes = Array.to_list (Array.init arity (fun _ -> newgenvar ())) in
      let to_unify = newgenty (Ttuple subtypes) in
      unify_exp_types loc env to_unify alpha;
      unify_exp_types loc env (List.nth subtypes field) beta;
      Ttypath_tuple (field, arity)

  | Ast_helper.Typath_list nth ->
      let nth = type_expect env nth Predef.type_int in
      unify_exp_types loc env alpha (newconstr Predef.path_list [beta]);
      Ttypath_list nth

  | Ast_helper.Typath_array nth ->
      let nth = type_expect env nth Predef.type_int in
      unify_exp_types loc env alpha (newconstr Predef.path_array [beta]);
      Ttypath_array nth
...
and type_argument ?recarg ?name env sarg ty_expected' ty_expected =
  ...
  | {desc; _} ->
      match match sarg.pexp_desc, desc, name with
        | Pexp_ident id, Tconstr(p, [], _), Some name
          when Path.same p Predef.path_unit && id.txt = Longident.parse name ->
          begin match Typetexp.find_value env sarg.pexp_loc id.txt with
          | exception Typetexp.Error (_, _, Typetexp.Unbound_value _) ->
            let arg =
              Ast_helper.Exp.construct (mknoloc (Longident.Lident "()")) None ~loc:sarg.pexp_loc
            in
            Some (type_argument env arg ty_expected' ty_expected)
          | _ ->
            None
          end
        | _ ->
        ...
        let sargs, more_sargs, arg =
          if ignore_labels && not (is_optional l) then begin
            ...
            if not optional || is_optional l' then
              ...
              Some (fun () -> option_some (type_argument ~name env sarg0
          ...
          with Not_found ->
            ...
            end else if has_non_labelled && not !Clflags.pure_caml && has_implicit ty then
...
and type_let ?(lazy_flag = NonLazy) ?bind ?(check = fun s -> Warnings.Unused_var s)
  ...
  let (pat_list, new_env, force, unpacks) =
    type_pattern_list ?bind env lazy_flag spatl scope nvs allow in
...
and type_implicit_arg sexp env loc ty =
  ...
  let str s = mk (Pexp_constant (Pconst_string (s, None))) in
  ...
  let call_site () =
    ...
    let fields =
      [mknoloc (Longident.parse "Mlfi_types.StackTrace.directory"),
       ...
       mknoloc (Longident.parse "filename"),
       str loc.Location.loc_start.Lexing.pos_fname;
       mknoloc (Longident.parse "line"),
...
let type_binding env rec_flag spat_sexp_list scope =
  ...
  let approx_tbl = Hashtbl.create 4 in
  List.iter
    (fun pvb ->
...
let rec full_name_mod env path =
  ...
  let open Path in
  let path = Env.normalize_path (Some Location.none) env path in
  match path with
  | Pident id ->
      if Ident.persistent id then Ident.name id
          ...
          let root =
            List.fold_left
              (fun root -> function
                 | {txt="#localmodule#"},_ -> "*LOCAL*"
                 | {txt="#funarg#"},_ -> "*FUNARG*"
                 | {txt="#root#"}, PTyp {ptyp_desc=Ptyp_var s} -> s
                 | _ -> root
...
let report_error env ppf = function
  ...
  | Lazy_let_complex_pattern ->
      fprintf ppf "Lazy let-binding can only use simple patterns."
  | Min_max_on_bad_type (op, ty) ->
      fprintf ppf "Operator '%s' cannot be applied on type %s." op ty
  | Modules_not_allowed ->
