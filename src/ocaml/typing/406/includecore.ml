...
let type_declarations ?equality ~loc env name decl1 id decl2 =
  if !Clflags.strict_props then
    let r = type_declarations ?equality ~loc env name decl1 id decl2 in
    if r = [] then []
    else begin
      Clflags.strict_props := false;
      Location.prerr_warning (Location.in_file !Location.input_name) (Warnings.Property_change name);
      Misc.try_finally
        (fun () -> type_declarations ?equality ~loc env name decl1 id decl2)
        ~always:(fun () -> Clflags.strict_props := true)
    end
  else
    type_declarations ?equality ~loc env name decl1 id decl2
