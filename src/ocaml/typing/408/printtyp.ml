...
let rec tree_of_class_type sch params =
  ...
  | Cty_signature sign ->
      ...
      in
      let all_vars =
        Vars.fold (fun l (m, v, t) all ->
          (l, m, v, t) :: all) sign.csig_vars []
      in
...
let still_in_type_group env' in_type_group item =
  match in_type_group, recursive_sigitem item with
    true, Some (_,Trec_next,_) -> env', true
...
and tree_of_signature_rec env' in_type_group = function
    [] -> []
  | item :: rem as items ->
      let env', in_type_group = still_in_type_group env' in_type_group item in
...
let print_items showval env x =
  ...
  | [] -> []
  | item :: rem as items ->
      let _, in_type_group = still_in_type_group env in_type_group item in
      let (sg, rem) = filter_rem_sig item rem in
      hide_rec_items items;
      protect_rec_items items;
      reset_naming_context ();
