  ...
  struct
    ...
    let rec fold_name f tbl acc =
      ...
      | Some {using = _; next; components} ->
          NameMap.fold
            (fun _name data acc -> List.fold_right f data acc)
            components
            acc
          |> fold_name f next
      | None ->
...
module IdTbl =
  struct
    ...
    let rec fold_name f tbl acc =
      ...
      match tbl.opened with
      | Some {root; using = _; next; components} ->
          NameMap.fold (fun name desc -> f name (Pdot (root, name), desc))
            components acc
          |> fold_name f next
      | None ->
          acc
...
let implicitly_used_persistent_ids = Hashtbl.create 8
let explicitly_used_persistent_ids = Hashtbl.create 8
...
and store_module ~check ~freshening_sub id addr presence md env =
  ...
  in
  let comps = Value (mod_comps, addr) in
  { env with
...
let add_components slot root env0 comps =
  ...
  let components, additions =
    ...
    in
    add_modules (fun x -> `Component x) components env0.components additions
  in
