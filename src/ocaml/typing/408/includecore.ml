...
let diff_props attrs1 attrs2 =
  ...
  if p1 <> p2 then begin
    List.iter
      (fun (k, v) -> Printf.printf "left:   %s -> %s\n%!" k v)
      p1;
    List.iter
      (fun (k, v) -> Printf.printf "right:  %s -> %s\n%!" k v)
      p2;
    true
  end
  else false
...
and compare_variants ~loc env params1 params2 n
  ...
  | cd1::rem1, cd2::rem2 ->
      ...
      else if diff_props cd1.cd_attributes cd2.cd_attributes then Some Type_properties
      else begin
        Builtin_attributes.check_alerts_inclusion
          ~def:cd1.cd_loc
          ~use:cd2.cd_loc
...
and compare_records ~loc env params1 params2 n
  ...
  | ld1::rem1, ld2::rem2 ->
      ...
      else if diff_props ld1.ld_attributes ld2.ld_attributes then Some Type_properties
...
let type_declarations ?equality ~loc env ~mark name decl1 id decl2 =
  if !Clflags.strict_props then
    let r = type_declarations ?equality ~loc env ~mark name decl1 id decl2 in
    if r = None then None
    else begin
      Clflags.strict_props := false;
      Location.prerr_warning (Location.in_file !Location.input_name) (Warnings.Property_change name);
      Misc.try_finally
        (fun () -> type_declarations ?equality ~loc env ~mark name decl1 id decl2)
        ~always:(fun () -> Clflags.strict_props := true)
    end
  else
    type_declarations ?equality ~loc env ~mark name decl1 id decl2
