...
let props_attributes env (attrs : attributes) =
  ...
  let really_approx_expr e =
    ...
    | Some s ->
        Some {e with pexp_desc=Pexp_constant(Pconst_string(s, Location.none, None))}
  in
  List.filter_map
    (fun a ->
...
let get_str_props attrs =
  List.map
    (List.map
       (function
         | (k, {pexp_desc=Pexp_constant (Pconst_string (s, _, _))}) -> (k, s)
         | _ -> assert false
       )
    )
...
let rec restore_props (sty : Parsetree.core_type) (ty : type_expr) : type_expr =
  let mk desc = Types.newty2 ~level:(get_level ty) desc in
  let ty =
...
let has_props sg =
  let default = Btype.type_iterators in
  ...
  let unmark () = Btype.unmark_iterators.Btype.it_signature Btype.unmark_iterators sg in
  match iter.Btype.it_signature iter sg with
  | () -> unmark (); false
  | exception Exit -> unmark (); true

let check_signature_for_cmi sg filename =
