...
module TypEq : sig
  ...
  let refl = Eq
  let trans (type a) (type b) (type c) (Eq : (a, b) t) (Eq : (b, c) t) = (Eq : (a, c) t)
...
type 'node gtype =
  ...
  | DT_polyvariant of (string * 'node gtype option * bool) list
...
let nonrec_map fnode f = function
  ...
  | DT_polyvariant fl ->
      let f = function None -> None | Some t -> Some (f t) in
      DT_polyvariant (List.map (fun (s, t, p) -> (s, f t, p)) fl)
...
let is_enumeration = function
  ...
  | DT_node{rec_descr = DT_variant {variant_constrs}; _} -> List.for_all (fun (_, _, args) -> args = C_tuple []) variant_constrs
  | _ -> false
...
let print_stype ?(print_node = fun _ _ -> None) ~show_enumerations ppf =
  let seen = Hashtbl.create 8 in
  let rec is_complex t =
    (show_enumerations || not (is_enumeration t)) &&
    match t with
    | DT_node n ->
        let already_seen = Hashtbl.mem seen n.rec_uid in
        begin match print_node already_seen n with
        | None -> not already_seen
        | Some _ -> false
        end
    | DT_list t
    | DT_array t
  ...
  let rec aux ppf t =
    ...
    | DT_node n ->
        ...
        | None ->
            if already_seen || (not show_enumerations && is_enumeration t) then name ppf n
            else begin
    ...
    | DT_polyvariant fl ->
        ...
        let all_present = List.for_all (fun (_, _, p) -> p) fl in
        let some_present = List.exists (fun (_, _, p) -> p) fl in
        if not all_present then Format.fprintf ppf "<";
        List.iteri
          (fun i (s, t, _) ->
             if i > 0 then Format.fprintf ppf "@ |";
             Format.fprintf ppf " `%s" s;
             match t with
             | Some t -> Format.fprintf ppf " of %a" aux t
             | None -> ()
          ) fl;
        if not all_present && some_present then begin
          Format.fprintf ppf "@ >";
          List.iter (function (s, _, true) -> Format.fprintf ppf " `%s" s | (_, _, false) -> ()) fl
        end;
        Format.fprintf ppf " ]@]";
...
let print_stype_hide_enumerations = print_stype ?print_node:None ~show_enumerations: false

let print_stype_custom print_node = print_stype ~print_node ~show_enumerations:true

let print_stype_ref = ref (print_stype ?print_node:None ~show_enumerations: true)
...
module Internal = struct
  ...
  let equal ~ignore_props ~ignore_path =
    ...
    and geq t1 t2 =
      ...
      | DT_polyvariant fl1, DT_polyvariant fl2 ->
          equal_list (fun (s1, t1, p1) (s2, t2, p2) ->
              s1 = s2 && p1 = p2 &&
              match t1, t2 with
              | None, None -> true
              | Some t1, Some t2 -> geq t1 t2
  ...
  let has_var t =
    ...
    and aux = function
      ...
      | DT_polyvariant fl -> List.exists (function (_, Some t, _) -> aux t | (_, None, _) -> false) fl
      | DT_node n -> node n
    in
    let r = aux t in
...
external ttype_of_stype: stype -> _ ttype = "%identity"

let rec remove_first_props = function
  | DT_prop(_, t) -> remove_first_props t
  | x -> x

let remove_first_props_ttype t = ttype_of_stype (remove_first_props (stype_of_ttype t))
let get_first_props t =
  let rec loop accu t =
    match t with
    | DT_prop (props, t) -> loop (props :: accu) t
    | _ -> List.flatten (List.rev accu)
  in
  loop [] t
let get_first_props_ttype t = get_first_props (stype_of_ttype t)
let add_props props t = if props <> [] then DT_prop(props, t) else t
let add_props_ttype props t = ttype_of_stype (add_props props (stype_of_ttype t))
...
let abstract_ttype t = ttype_of_stype (abstract_stype (stype_of_ttype t))

let split_arrow_ttype t =
  match remove_first_props (stype_of_ttype t) with
  | DT_arrow(_, t1, t2) -> ttype_of_stype t1, ttype_of_stype t2
  | _ -> assert false
...
let ttypes_equality t1 t2 =
  if types_equality (stype_of_ttype t1) (stype_of_ttype t2) then Some TypEq.unsafe
  else None

let ttypes_equality_modulo_props t1 t2 =
  if types_equality_modulo_props (stype_of_ttype t1) (stype_of_ttype t2) then Some TypEq.unsafe
  else None

let build_arrow_ttype t1 t2 =
  ttype_of_stype (DT_arrow("", stype_of_ttype t1, stype_of_ttype t2))

let ttype_fst t = match stype_of_ttype t with DT_tuple [t; _] -> ttype_of_stype t | _ -> assert false
let ttype_snd t = match stype_of_ttype t with DT_tuple [_; t] -> ttype_of_stype t | _ -> assert false
...
module StackTrace = struct
  ...
  let print_call_site ppf cs =
    Format.fprintf ppf "%s:%i" cs.filename cs.line
end
