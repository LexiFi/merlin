...
type 'node gtype =
  ...
  | DT_object of (string * 'node gtype) list (** sorted by method name *)
  | DT_polyvariant of (string * 'node gtype option * bool) list (** sorted by constructor name *)
...
val print_stype_custom: (bool -> node -> string option) -> Format.formatter -> stype -> unit
...
val get_first_props: stype -> (string * string) list
val get_first_props_ttype: 'a ttype -> (string * string) list

val add_props: (string * string) list -> stype -> stype
(** Add a toplevel Tprops node on top of a stype, if the list of extra properties
    is non empty (otherwise return the original stype). *)

val add_props_ttype: (string * string) list -> 'a ttype -> 'a ttype
(** Same as [add_props] for ttypes. *)
...
[@@alert old_call_site_api "This feature is deprecated and may be removed in the future."]
