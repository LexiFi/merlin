    ...
    let visited =
      match ty.desc with
        Tarrow _ | Ttuple _ | Tvariant _ | Tconstr _ | Tobject _ | Tpackage _ ->
          (ty,(t,bound)) :: visited
      | Tvar _ | Tfield _ | Tnil | Tpoly _ | Tunivar _ | Tlink _ | Tsubst _ | Tprop _ ->
          visited
    in
              ...
              if co then
                if cn then
                  if !Clflags.pure_caml then
                    ...
                    subtype_rec env (Trace.diff t2 t1::trace) t2 t1
        ...
        with Not_found ->
          ...
          try
            ...
            let cmp_records cstrs fields1 fields2 =
              ...
              List.fold_left2
                (fun cstrs l1 l2 ->
                   ...
                   let cstrs = subtype_rec env (Trace.diff t1 t2::trace) t1 t2 cstrs in
                   if l1.ld_mutable = Mutable then
                     subtype_rec env (Trace.diff t2 t1::trace) t2 t1 cstrs
                   else
                     cstrs
                )
            ...
            let cstrs =
              ...
              | Type_variant constrs1, Type_variant constrs2
