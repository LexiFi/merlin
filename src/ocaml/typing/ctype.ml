              ...
              if co then
                if cn then
                  if !Clflags.pure_caml then
                    ...
                    subtype_rec env (Subtype.diff t2 t1::trace) t2 t1
            ...
            let cmp_records cstrs fields1 fields2 =
              ...
              List.fold_left2
                (fun cstrs l1 l2 ->
                   ...
                   let cstrs = subtype_rec env (Subtype.diff t1 t2::trace) t1 t2 cstrs in
                   if l1.ld_mutable = Mutable then
                     subtype_rec env (Subtype.diff t2 t1::trace) t2 t1 cstrs
                   else
                     cstrs
                )
